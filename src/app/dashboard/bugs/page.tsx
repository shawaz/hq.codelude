 ...
 * Franchiseen Mobile reconciled 8 Sep 2026 against /Users/shawaz/Developer/franchiseen_expo
 * @ b0cb13d — tsc, expo lint, supabase_setup.sql and the Supabase call sites.
 */
const DATA: PlatformBugs[] = [
  {
    ...
    bugs: [
      {
        title: 'Live API keys committed in eas.json',
        area: 'eas.json — build.production.env',
        severity: 'high',
        status: 'open',
        detail: 'Supabase anon, Clerk pk_live, Stripe pk_live and a Google Maps key are all checked into the repo. The first three are publishable and ship in the bundle anyway, so committing them is hygiene rather than breach. The Google Maps key is not — AIza keys are billable, and unless it is restricted by bundle ID in Google Cloud Console anyone can spend against the quota. Verify the restriction first, then rotate and move all four to EAS secrets.',
      },
      {
        title: 'Only 2 of ~15 tables have schema and RLS in version control',
        area: 'supabase_setup.sql',
        severity: 'high',
        status: 'open',
        detail: 'The file creates and secures properties and property_shops. The client also reads and writes businesses, deals, profiles, products, payments, notifications, support_tickets and job_positions — none of which appear in the repo, so their RLS status cannot be verified from source. Combined with a publishable anon key, any of those tables left unprotected is writable by anyone who extracts the key. Admin approval writes (businesses.status = approved, deals.status = live) run client-side against exactly these tables.',
      },
      {
        title: 'RLS policies are ownership-blind',
        area: 'supabase_setup.sql — properties_update, shops_update',
        severity: 'high',
        status: 'open',
        detail: "Both UPDATE policies are USING ((auth.jwt() ->> 'sub') IS NOT NULL) — they check that a caller is signed in, not that they own the row. Any authenticated user can update any property or any shop unit, including approving their own listing. The SELECT policy on property_shops does scope by submitted_by, so the pattern is understood; it just was not applied to writes.",
      },
      {
        title: 'Two competing Supabase auth patterns, and the wrong one is more widely used',
        area: 'lib/supabase.ts vs ~10 screens',
        severity: 'medium',
        status: 'open',
        detail: "createAuthClient exists precisely because, in its own words, setSession 'conflicts with persistSession: true and autoRefreshToken: true on the shared client'. Ten screens still call supabase.auth.setSession({ access_token, refresh_token: '' }) on the shared client anyway. An empty refresh token against autoRefreshToken: true is the likely mechanism behind the repeated RLS fixes in the commit log. Standardise on createAuthClient for authenticated calls.",
      },
      {
        title: 'catch() on a Supabase thenable never attaches',
        area: 'agent/select-building.tsx:55, user/checkout.tsx:141',
        severity: 'medium',
        status: 'open',
        detail: 'PostgrestBuilder.then() returns PromiseLike<void>, which has no catch. These are the only two errors a real typecheck reports. In select-building the consequence is user-visible: a failed query never runs setLoading(false), so the spinner hangs forever. Await the query and branch on the returned error instead.',
      },
      {
        title: 'tsc_output.txt is a stale artifact reporting 453 phantom errors',
        area: 'tsc_output.txt',
        severity: 'low',
        status: 'open',
        detail: 'Generated by running tsc without the project tsconfig, so the @/* path aliases never resolved and JSX was unconfigured. It reports 453 errors including nine missing modules that all exist on disk. A real tsc --noEmit -p tsconfig.json reports 2. Anyone triaging from this file wastes the session; delete it.',
      },
    ],
  },
  {

...
const LEGAL_STEPS = [
  {
    phase: 'Entity Setup', color: '#dbdbdb',
    steps: [
  ...
  {
    phase: 'Round Structure', color: '#eeeeee',
    steps: [
  ...
  {
    phase: 'Site & Regulatory', color: '#b5b5b5',
    steps: [
  ...
  {
    phase: 'FEMA (Foreign Investment)', color: '#a5a5a5',
    steps: [
...
const TIMELINE = [
  ...
  { month: 'Jun 2026',     phase: 'Site Proof',   color: '#b5b5b5', items: ['Complete Mangaluru coastal site survey', 'Thermal feasibility study commissioned', 'CRZ permit pathway analysis initiated', 'Site photos and documentation for investor deck'] },
  { month: 'Jul–Aug 2026', phase: 'Outreach',     color: '#eeeeee', items: ['Approach Karnataka family offices (warm intros)', 'Submit IAN / Mumbai Angels application', 'First investor meetings', 'Share financial model and site survey outcomes'] },
  { month: 'Aug–Sep 2026', phase: 'Due Diligence',color: '#adadad', items: ['CCD term sheet issued to interested investors', 'Shareholder agreement negotiation', 'Investor due diligence on land, permits, feasibility', 'Valuation report from registered CA/valuer'] },
  { month: 'Oct 2026',     phase: 'First Close',  color: '#dbdbdb', items: ['First close: target ₹8–10 Cr from anchor investors', 'SHA signed, CCDs allotted', 'RBI FC-GPR filing if foreign investors', 'Escrow account funded'] },
  { month: 'Nov–Dec 2026', phase: 'Final Close',  color: '#dbdbdb', items: ['Final close: reach ₹18.1 Cr target', 'CRZ permits filed (parallel track)', 'Engineering contractor shortlist', 'MESCOM HT application submitted'] },
  { month: 'Q1 2027',      phase: 'Deploy',       color: '#c8c8c8', items: ['Construction commences (Building A + Thermal loop)', 'Working capital deployed for site operations', 'Pilot compute tenant onboarded', 'Series A preparation begins'] },
];
...
const CRISIS_STATS = [
  { label: 'Avg. data center water draw',  val: '1.8 L / kWh',  sub: 'Direct cooling water per kWh of IT load (EESI / TechTarget)', color: '#9d9d9d' },
  { label: 'AI vs. traditional clusters',  val: '10–50×',       sub: 'More cooling water consumed by AI training/inference vs. legacy server farms', color: '#b5b5b5' },
  { label: 'US projects blocked/delayed',  val: '$64B+',        sub: '48+ data center projects stalled by local opposition in 2025 — water & grid strain are the lead causes (Data Center Watch)', color: '#eeeeee' },
  { label: 'Roborns freshwater draw',      val: 'Zero',         sub: 'Seawater-cooled — fresh water is the byproduct, not the input', color: '#dbdbdb' },
];
...
const HYPERSCALER_TARGETS = [
  ...
  {
    category: 'Microsoft', color: '#eeeeee',
  ...
  {
    category: 'Amazon (AWS)', color: '#c8c8c8',
...
const GOVT_PIPELINE = [
  {
    phase: 'Pre-requisite — Credibility Gate', color: '#9d9d9d',
    steps: [
  ...
  {
    phase: 'National — Compute & AI', color: '#dbdbdb',
    steps: [
  ...
  {
    phase: 'National — Cleantech & Water', color: '#eeeeee',
    steps: [
  ...
  {
    phase: 'State — Karnataka', color: '#b5b5b5',
    steps: [
    ...
    >
      {({ venture }) => {
        ...
        return (
          <>
      {/* Summary strip */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,1fr)', gap: '1px', background: 'var(--card-border)', border: '1px solid var(--card-border)', marginBottom: '1.5rem', borderLeft: '2px solid #dbdbdb' }}>
        {[
          { label: 'Target raise',  val: '₹18.1 Cr', sub: '~$2.1M USD',          color: '#dbdbdb' },
          { label: 'Pre-money',     val: '₹60 Cr',   sub: '~$7.1M valuation',    color: 'var(--off-white)' },
          { label: 'Dilution',      val: '23.2%',     sub: 'Seed investors',       color: '#b5b5b5' },
          { label: 'Instrument',    val: 'CCDs',      sub: 'Converts at Series A', color: '#eeeeee' },
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                {ROUND.rights.map((r, i) => (
                  <div key={i} style={{ display: 'flex', gap: '0.6rem', alignItems: 'flex-start' }}>
                    <span style={{ color: sc('#dbdbdb'), fontFamily: 'var(--font-mono)', fontSize: '0.65rem', flexShrink: 0 }}>—</span>
                  ...
                  <div key={i} style={{ background: 'var(--card-bg)', padding: '0.85rem 1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.35rem' }}>
                      ...
                      <div style={{ display: 'flex', gap: '1rem' }}>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: sc('#dbdbdb'), fontWeight: 700 }}>{s.multiple}</span>
                        <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.68rem', color: sc('#b5b5b5') }}>{s.irr} IRR</span>
                      </div>
                    </div>
                ...
                {USE_OF_FUNDS.map((u, i) => (
                  <div key={i} style={{ background: 'var(--card-bg)', padding: '0.85rem 1rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.35rem' }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.72rem', color: sc('#dbdbdb'), fontWeight: 700, flexShrink: 0, marginLeft: '1rem' }}>{u.amount}</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div style={{ flex: 1, height: 3, background: 'var(--card-border)', borderRadius: 2 }}>
                        <div style={{ height: '100%', width: `${u.pct}%`, background: '#dbdbdb', borderRadius: 2 }} />
                      </div>
            ...
            <div key={pi} style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'var(--card-border)', border: '1px solid var(--card-border)' }}>
                {phase.steps.map((s, si) => {
                  const statusColor = s.status === 'required' ? '#9d9d9d' : s.status === 'recommended' ? '#b5b5b5' : 'var(--muted)';
                  return (
                    <div key={si} style={{ background: 'var(--card-bg)', padding: '1rem 1.25rem', display: 'grid', gridTemplateColumns: '1fr 120px 110px', gap: '1.25rem', alignItems: 'start' }}>
                      ...
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.62rem', color: sc('#b5b5b5'), lineHeight: 1.5 }}>{s.cost}</div>
        ...
        <div>
          ...
          {GOVT_PIPELINE.map((phase, pi) => (
            <div key={pi} style={{ marginBottom: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1px', background: 'var(--card-border)', border: '1px solid var(--card-border)' }}>
                {phase.steps.map((s, si) => {
                  const statusColor = s.status === 'required' ? '#9d9d9d' : s.status === 'apply now' ? '#dbdbdb' : s.status === 'recommended' ? '#b5b5b5' : 'var(--muted)';
                  return (

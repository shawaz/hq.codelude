...
export default function OffboardingPage() {
  const steps = [
    ...
    { phase: 'Within 24 hours', items: ['Disable Google Workspace account (retain email for 30 days, then delete)', 'Remove from GitHub organisation', 'Remove from HQ → Team (revoke their @codelude.com Google access)', 'Remove from all WhatsApp / Telegram groups', 'Revoke 2FA tokens and access manager entries'] },
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        {steps.map((s, i) => (
          <div key={i} style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', padding: '1.5rem', borderLeft: '2px solid #adadad' }}>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: sc('#adadad'), letterSpacing: '0.14em', textTransform: 'uppercase', marginBottom: '0.75rem' }}>{s.phase}</div>

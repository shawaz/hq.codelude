          ...
          {INVESTOR_ROUNDS.map(r => {
            ...
            return (
              <div key={r.id} onClick={() => setSelected(isActive ? null : r.id)}
                style={{ background: isActive ? '#131311' : 'var(--card-bg)', padding: '1.25rem 1.5rem', cursor: 'pointer', borderLeft: isActive ? `2px solid ${VENTURE_COLORS[r.venture]}` : '2px solid transparent', transition: 'background 0.15s' }}>
